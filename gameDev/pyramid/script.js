...
// объявляем переменные ThreeJS - камеру, сцену, рендер
let camera, scene, renderer;

// объявляем мир CannonJS
let world;

// время последней анимации
let lastTime;

// тут будем хранить уже стоящие части пирамиды
let stack;

// падающие части деталей, которые не уместились в границы пирамиды
let overhangs;

// высота каждой детали
const boxHeight = 1;

// исходная ширина и высота каждой детали
...
// получаем доступ на странице к разделам с очками, правилами и результатом
...
function addLayer(x, z, width, depth, direction) {
    ...
    // создаем новый слой на этой высоте
...
function generateBox(x, y, z, width, depth, falls) {
    ...
    // создаем цвет, материал и полигональную сетку, которая создаст нам коробку
    ...
    // создаем новый виртуальный блок, который совпадает с отрисованным на прошлом этапе
    ...
    // создаем новую фигуру на основе блока
...
function addOverhang(x, z, width, depth) {
    ...
    // создаем новую фигуру, которая вышла за свес
    const overhang = generateBox(x, y, z, width, depth, true);

    // добавляем ее в свой массив
...
function cutBox(topLayer, overlap, size, delta) {
    ...
    // обновляем верхинй блок в CannonJS
...
// подготоаливаемся к запуску и показываем демку на автопилоте
init();
...
function init() {
    ...
    // задаем точность игры на автопилоте
...
function startGame() {
    ...
    // если видны очки, то обнуляем их
    ...
    // если сцена уже есть, тоже убираем
    if (scene) {
        ...
        // и первый слой
        addLayer(-6, 0, originalBoxSize, originalBoxSize, "x");
    }

    // если уже есть камера - сбрасываем ее настройки
    if (camera) {
        camera.position.set(4, 4, 4);
        camera.lookAt(0, 0, 0);
    }
...
window.addEventListener("keydown", function (event) {
    ...
    // если нажата R (в русской или англ. раскладке)
...
function splitBlockAndAddNextOneIfOverlaps() {
    ...
    // берем верхний блок и тот, что под ним
    ...
    // если двигались по оси Х, то берем ширину блока, а если нет (по оси Z) - то глубину
    ...
    if (overlap > 0) {
        ...
        // если свес был по оси Х, то получаем ширину, если по Z - глубину
        const overhangWidth = direction == "x" ? overhangSize : topLayer.width;
        const overhangDepth = direction == "z" ? overhangSize : topLayer.depth;

        // рисуем новую фигуру после обрезки, которая будет падать вниз
        ...
        // если идет подсчет очков - выводим текущее значение
        if (scoreElement) scoreElement.innerText = stack.length - 1;

        // добавляем в сцену новый блок
        addLayer(nextX, nextZ, newWidth, newDepth, nextDirection);

    // если нет свеса и игрок полностью промахнулся мимо пирамиды
    } else {
        // обрабатываем промах
        missedTheSpot();
    }
...
function missedTheSpot() {
    ...
    // формируем срез (который упадет) полностью из всего блока
    addOverhang(
        topLayer.threejs.position.x,
        topLayer.threejs.position.z,
        topLayer.width,
    ...
    // помечаем, что наступл конец игры
    gameEnded = true;
...
function animation(time) {
    ...
    if (lastTime) {
        ...
        // берем верхний и предыдущий слой
        const topLayer = stack[stack.length - 1];
        const previousLayer = stack[stack.length - 2];
        ...
        // ЕСЛИ не конец игры
        // И это не автопилот
        // ИЛИ это все же автопилот, но алгоритм еще не довел блок до нужного места
        const boxShouldMove = 
            !gameEnded &&
            (!autopilot ||
                (autopilot && 
...
// обрабатываем изменения размера окна браузера
